...
class DecisionTree:
    ...
    def train(self, dataframe, class_series, attrs):   
        if len(set(class_series)) == 1:
            #print(list(class_series.iloc[:1]))
        ...
        if len(attrs) >= 1:
            ...
            for attribute in attrs:
                entropy = 0     
                for value in list(set(dataframe[attribute])):       #kanske är bra med attributes
        ...
        for value in list(set(dataframe[next_node])):
            ...
            if len(attrs) >= 2:
                dataframe2 = dataframe.drop(next_node, axis = 1).loc[Index] #make these two just one line
                #dataframe2 = dataframe2.loc[Index]
            else:
    ...
    def predict(self, instance, decisiontree):
        ...
        value = int(instance[decisiontree.root]) #remaking the series-value into a string
    ...
    def make_tree_dictionary(self, trained_class):
        ...
        if trained_class.root != None:
            ...
            for i in range(len(subtree)):
                treelist.append([i + 1, self.make_tree_dictionary(subtree[i][2])])
            #print(treelist)
